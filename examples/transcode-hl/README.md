...
- Creating a `av.Decoder` from a stream with `av.NewDecoder`, which finds, configures, and opens the codec in one call
...
- Adding an output stream with `output.AddStream(encoder)`, which copies codec parameters and time base automatically
