...
Build the project using `just build`. Using `just build` is the only valid way to build. Never build the project using adhoc commands. Never rediect builds elsewhere, the build system does the right thing for you. Work with the provide build tool, not against it.
